...
WITH vendor_products AS (
    SELECT DISTINCT
        v.vendor_name,
        p.product_name,
        vi.original_price AS price_per_unit
...
SELECT 
    vendor_name,
    product_name,
    SUM(price_per_unit * 5) AS total_revenue
...
SET current_quantity = (
    ...
    WHERE vi.product_id = product_units.product_id
      AND vi.market_date = (
          SELECT MAX(sub_vi.market_date) 
          FROM vendor_inventory sub_vi
          WHERE sub_vi.vendor_id = vi.vendor_id
            AND sub_vi.product_id = vi.product_id
      )
);
